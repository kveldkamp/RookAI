...
package nazarenerook;

import java.util.*;
//enums
import static nazarenerook.Card.Suit;
...
public class NPC extends Player {
    ...
    public ArrayList<Card> winningCards()
    {
        ...
        for(int i = 0; i <= 44; i++) {
            deck[i] = new Card();
            if(i <= 10) {
                deck[i].setCard(Suit.RED, i);
            } else if(i <= 21){
                deck[i].setCard(Suit.BLUE, i);
            } else if (i <=32) {
                deck[i].setCard(Suit.GREEN, i);
            } else if (i <= 43) {
                deck[i].setCard(Suit.BLACK, i);
            } else if (i == 44)
        ...
        if(deck[44] != null)
        {
        if(highestGreen != noSuitLeft && trumpSuit == Suit.GREEN && deck[highestGreen].getRank() <  RookRank)
            highestGreen = RookValue;
        if(highestRed != noSuitLeft && trumpSuit == Suit.RED && deck[highestRed].getRank() <  RookRank)
            highestRed = RookValue;
        if(highestBlue != noSuitLeft && trumpSuit == Suit.BLUE && deck[highestBlue].getRank() <  RookRank)
            highestBlue = RookValue;
        if(highestBlack != noSuitLeft && trumpSuit == Suit.BLACK && deck[highestBlack].getRank() <  RookRank)
    ...
    */
    ...
    public int GetThrowAwayBidTeamLeadCard()
    {
        ...
        for(int i = 0; i < 15; i++)
        {
            if(hand[i] != null)
            {
                if(hand[i].getSuit() != trumpSuit)
                {
                        if(hand[i].getSuit() == Suit.RED)
                            numRed++;
                        if(hand[i].getSuit() == Suit.BLUE)
                            numBlue++;
                        if(hand[i].getSuit() == Suit.GREEN)
                            numGreen++;
                        if(hand[i].getSuit() == Suit.BLACK)
                            numBlack++;
                    if(hand[i].getScore() == 0)
                    {
                        if(lowestRed == -1 && hand[i].getSuit() == Suit.RED)
                            lowestRed = i;
                        if(lowestBlue == -1 && hand[i].getSuit() == Suit.BLUE)
                            lowestBlue = i;
                        if(lowestGreen == -1 && hand[i].getSuit() == Suit.GREEN)
                            lowestGreen = i;
                        if(lowestBlack == -1 && hand[i].getSuit() == Suit.BLACK)
                            lowestBlack = i;
                    }
                }
            }
     ...
     */
    private Suit determineSecondLongestSuit()
    {
        int second = 0;
        Suit secondLongest = Suit.NOSUIT;
        boolean suitsOfEqualLength = determineSuitsOfEqualLengthPresent();
            if(strongestSuit == Suit.RED)
            {
                ...
                else
                {
                    if(blueLength == greenLength)
                        second = Math.max(computeSuitStrength(Suit.BLUE), computeSuitStrength(Suit.GREEN));
                    else if(blueLength == blackLength)
                        second = Math.max(computeSuitStrength(Suit.BLUE), computeSuitStrength(Suit.BLACK));
                    else if(greenLength == blackLength)
                        second = Math.max(computeSuitStrength(Suit.GREEN), computeSuitStrength(Suit.BLACK));
                }

            }
            else if(strongestSuit == Suit.GREEN)
            {
                if(!suitsOfEqualLength)
                {
                ...
                else
                {
                    if(redLength == blueLength)
                        second = Math.max(computeSuitStrength(Suit.BLUE), computeSuitStrength(Suit.RED));
                    else if(redLength == blackLength)
                        second = Math.max(computeSuitStrength(Suit.RED), computeSuitStrength(Suit.BLACK));
                    else if(blueLength == blackLength)
                        second = Math.max(computeSuitStrength(Suit.BLUE), computeSuitStrength(Suit.BLACK));
                }
            }
            else if(strongestSuit == Suit.BLUE)
            {
                if(!suitsOfEqualLength)
                {
                ...
                else
                {
                    if(redLength == greenLength)
                        second = Math.max(computeSuitStrength(Suit.GREEN), computeSuitStrength(Suit.RED));
                    else if(redLength == blackLength)
                        second = Math.max(computeSuitStrength(Suit.RED), computeSuitStrength(Suit.BLACK));
                    else if(greenLength == blackLength)
                        second = Math.max(computeSuitStrength(Suit.GREEN), computeSuitStrength(Suit.BLACK));
            ...
            else if(strongestSuit == Suit.BLACK)
            {
                if(!suitsOfEqualLength)
                {
                    second = Math.max(redLength, greenLength);
                ...
                else
                {
                    if(redLength == greenLength)
                        ...
                        second = Math.max(computeSuitStrength(Suit.RED), computeSuitStrength(Suit.BLUE));
                    else if(greenLength == blueLength)
                        second = Math.max(computeSuitStrength(Suit.GREEN), computeSuitStrength(Suit.BLUE));
                }
            }
        if(!suitsOfEqualLength)
        {
            if(redLength == second && strongestSuit != Suit.RED)
                secondLongest = Suit.RED;
            else if(greenLength == second && strongestSuit != Suit.GREEN)
                secondLongest = Suit.GREEN;
            else if(blueLength == second && strongestSuit != Suit.BLUE)
                secondLongest = Suit.BLUE;
            else if(blackLength == second && strongestSuit != Suit.BLACK)
                secondLongest = Suit.BLACK;
        }
        else
        {
            if(computeSuitStrength(Suit.RED) == second && strongestSuit != Suit.RED)
                secondLongest = Suit.RED;
            else if(computeSuitStrength(Suit.GREEN) == second && strongestSuit != Suit.GREEN)
                secondLongest = Suit.GREEN;
            else if(computeSuitStrength(Suit.BLUE) == second && strongestSuit != Suit.BLUE)
                secondLongest = Suit.BLUE;
            else if(computeSuitStrength(Suit.BLACK) == second && strongestSuit != Suit.BLACK)
                secondLongest = Suit.BLACK;
        }
    ...
    {
        Suit lastWinnerSuit = strongestSuit;
